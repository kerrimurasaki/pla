...
- [x] **F1 — shortcut_check was self-reported.** Fixed (2026-07-07): `judgePracticeItem` in [src/validators/itemJudge.ts](src/validators/itemJudge.ts) — an independent adversarial LLM pass ("you are the check, not the author") testing doing-vs-describing, skill-only path, and response concreteness. Wired into `generateDiagnostic` after the deterministic validators; rejections feed the regeneration retry. Unparseable judge output counts as rejection (never wave an item through because the check glitched). Cost: one extra LLM round for diagnostics (judges run concurrently like the generations).
- [x] **F2 — placeholder leak.** Fixed: deterministic bracket-placeholder check in `validatePracticeItem` (plus F1's concreteness question as the semantic backstop).
- [x] **F3 — extraction granularity.** Fixed: extraction prompt now demands verb-first teachable capabilities and forbids phase/heading nouns; single-word evidence quotes rejected deterministically; extraction got the same retry-with-fed-back-errors loop as the other generators.
- [ ] **F4 — classification skew:** 23/25 components were cognitive_routine; zero comparatives/correlated_features. Needs `npm run eval:classifier` against the real provider to quantify (requires API key — Kerri or a key in env). Prompt likely needs boundary examples for non-routine types.
- [x] **F5 — prerequisite noise.** Fixed: prereq prompt rule + deterministic strip — a composite's own components are never its prerequisites.
- [ ] **F6 — semantic near-duplicates:** `testing` vs `software_testing`, `requirements_analysis` vs `requirements_gathering`. Slug dedup can't see synonyms — needs a merge pass before graph assembly. (Extraction prompt now says "merge near-synonyms", which helps within one call, but components from separate parallel decompositions can't see each other.)
...
- [x] **Second timeout (2026-07-07): the F1 adversarial-judge round pushed the same short job ad past 60s** (`FUNCTION_INVOCATION_TIMEOUT` at 60.5s — quality checks cost latency, as expected). All three remaining levers pulled: `maxDuration` raised 60→300 (Hobby allows 300s with Fluid Compute, default-on for new projects — **if it still dies at exactly ~60s, check the Vercel project's Fluid Compute setting**); taxonomy mapping now runs concurrently with graph building; `includeDiagnostics: false` request flag + UI checkbox for a fast graph-only run.
